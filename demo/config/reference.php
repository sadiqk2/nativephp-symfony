 ...
 *     }>,
 * }
 * @psalm-type NativeDesktopConfig = array{
 *     app_id?: scalar|Param|null, // Reverse-domain application id. The runtime passes it to setAppUserModelId(). // Default: "com.example.app"
 *     name?: scalar|Param|null, // Human-readable app name. Slugged for the package filename. // Default: "App"
 *     version?: scalar|Param|null, // Default: "1.0.0"
 *     author?: scalar|Param|null, // Default: null
 *     copyright?: scalar|Param|null, // Default: null
 *     description?: scalar|Param|null, // Default: null
 *     website?: scalar|Param|null, // Default: null
 *     deeplink_scheme?: scalar|Param|null, // Custom URL scheme, without "://". Enables the OpenedFromURL event. // Default: null
 *     base_url?: scalar|Param|null, // Fallback base URL for building absolute window URLs outside an HTTP request. // Default: null
 *     updater?: array{
 *         enabled?: bool|Param, // Default: false
 *         default?: scalar|Param|null, // Default: "github"
 *         providers?: array<string, mixed>,
 *     },
 *     php_ini?: array<string, scalar|Param|null>,
 *     events?: array{
 *         allowed_namespaces?: list<scalar|Param|null>,
 *     },
 *     prebuild?: list<scalar|Param|null>,
 *     postbuild?: list<scalar|Param|null>,
 *     nsis?: array{
 *         delete_app_data_on_uninstall?: bool|Param, // Default: false
 *     },
 *     build?: array{
 *         exclude?: list<scalar|Param|null>,
 *         keep?: list<scalar|Param|null>,
 *         env_remove?: list<scalar|Param|null>,
 *         env_keep?: list<scalar|Param|null>,
 *         env_defaults?: array<string, scalar|Param|null>,
 *     },
 ...
 *     block_browser_access?: bool|Param, // Reject requests that carry neither the _php_native cookie nor the X-NativePHP-Secret header while running inside the runtime. Leave this on: the app is served on a real loopback port and the secret is the only thing keeping other local processes out. Turning it off also switches off exempt_runtime_firewall, which hands your access_control away only while this gate is enforcing in its place — so an app with a rule of ^/ then stops booting, which native:doctor reports. // Default: true
 *     exempt_runtime_firewall?: bool|Param, // Keep the application's own access_control off /_native/api/ while running inside the runtime, by decorating security.access_map. Without it a rule of ^/ answers POST /_native/api/booted with a 401, the runtime discards it, and the app boots to a window that never does anything. Outside the runtime your firewall still applies in full — that path dispatches events by name, so it must stay protected on the web. Only applies when the security bundle is installed and block_browser_access is on, since the exemption relies on the shared secret being checked in place of the rules it hands away. // Default: true
 * }
 ...
 *     name?: scalar|Param|null, // Default: "App"
 *     version?: scalar|Param|null, // Must be a non-empty string. YAML `version: 1.0` is a float and the container coerces it to "1" — and the hosts compare this by string equality, so a coerced value silently disables the runtime route dump. Quote it. // Default: "1.0.0"
 *     fake_bridge?: bool|Param, // Swap the native bridge for a recording fake. The real bridge is a compiled PHP extension that only exists inside a packaged app, so this is how the mobile API is exercised in a test suite or in a browser during development. // Default: false
 *     platform?: scalar|Param|null, // ios or android, for resolving platform-variant style classes. Null drops every ios:/android: class, which is the safe default off a device but wrong on one — set it per request from the device info if you use those variants. // Default: null
 *     collect_garbage?: bool|Param, // Run gc_collect_cycles() after each request in persistent mode. Costs time per request; worth it only for an app that holds large object graphs. // Default: false
 * }
 ...
 *     native_desktop?: NativeDesktopConfig,
 *     native_mobile?: NativeMobileConfig,
 *     "when@dev"?: array{
 *         imports?: ImportsConfig,
 *         parameters?: ParametersConfig,
 *         services?: ServicesConfig,
