...
| `native:doctor` reported `✗ does not route` for both runtime endpoints on a correctly wired application, on any symfony/routing below 7.4 — the one failure a diagnostic must never have, since the reader then rewrites a routes file that was already right | It matched a `Request::create($path, 'POST')` without setting the method on the router's *context*, and `UrlMatcher` only began taking it from the request in 7.4 and 8.1. Found by adding `composer update --prefer-lowest` to CI before publishing: the declared range was a promise nobody had checked, and it was false in three places |
| `ProjectPath::absolute()` answered two different things across the declared range while its own docblock claimed one — `{project}/C:\dev` on new Symfony, `{project}/C:/dev` on old | `Path::join()` stopped rewriting backslashes in symfony/filesystem 7.4 and 8.1, and the method had been leaning on it to normalise. It normalises explicitly now, in both bundles, so the answer is not the resolver's business |
